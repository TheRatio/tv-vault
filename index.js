...
const start = async () => {
	...
	function downloadFile(url) {
		return new Promise((resolve, reject) => {
			const file = fs.createWriteStream(config.downloadPath, { flags: "wx" });

			const downloadrequest = request(url, (error, response) => {
				if (response.statusCode === 200) {
					response.pipe(file);
				} else {
					file.close();
					fs.unlink(config.downloadPath, () => {}); // Delete temp file
